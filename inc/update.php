 ...
 *   1. Bump the version (header + CH_MAILDEL_VERSION) and push.
 ...
 *      newer than the installed CH_MAILDEL_VERSION, and show the normal
 ...
 *
 *     define( 'CH_MAILDEL_GH_TOKEN', 'github_pat_...' );  // Contents: read
 */
...
class CH_MailDel_Updater {

	/** @var string Absolute path to the main plugin file. */
	private $file;

	/** @var string e.g. "ch-mail-delivery/ch-mail-delivery.php". */
	private $basename;

	/** @var string Plugin folder slug, e.g. "ch-mail-delivery". */
	private $slug;
	...
	const CACHE_KEY = 'ch_maildel_update_release';
	const CACHE_TTL = 6 * HOUR_IN_SECONDS;

	public function __construct( $file, $username, $repository ) {
		$this->file       = $file;
		$this->basename   = plugin_basename( $file );
		$this->slug       = dirname( $this->basename ); // "ch-mail-delivery"
	...
	private function token() {
		return defined( 'CH_MAILDEL_GH_TOKEN' ) && CH_MAILDEL_GH_TOKEN ? CH_MAILDEL_GH_TOKEN : '';
	}
	...
	private function get_release() {
		...
		$headers = array(
			...
			'User-Agent' => 'ch-mail-delivery/' . CH_MAILDEL_VERSION . '; ' . home_url(),
		);
		if ( $this->token() ) {
...
if ( is_admin() || ( defined( 'DOING_CRON' ) && DOING_CRON ) ) {
	new CH_MailDel_Updater( CH_MAILDEL_FILE, 'connormesec', 'ch-mail-delivery' );
}
